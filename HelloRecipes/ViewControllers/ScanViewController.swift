...
//        toggleTimer()
    ...
    /// Freezes the capture session and posts a notification for the ObjectSelectionViewController
    ...
    /// Checks the status of the camera and reacts accordingly
    fileprivate func checkCameraStatus() {
        // Camera
        ...
        case .notDetermined: // The user has not yet been asked for camera access
            AVCaptureDevice.requestAccess(for: .video) { (success) in
                if success {
                    print("Successfully got access to camera!")
                } else {
                    self.presentAlertForCameraAccess()
                }
            }
        case .denied:
            presentAlertForCameraAccess()
        case .restricted:
            fatalError()
        }
    }
    ...
    /// Sets the color for the UI based on the theme (When user tapped on navigation bar)
 ...
 extension ScanViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        Timer.scheduledTimer(withTimeInterval: 6, repeats: true, block: { (_) in
 ...
 // MARK: - Alert when user denies camera access
 extension ScanViewController {
    func presentAlertForCameraAccess() {
        let alert = UIAlertController(title: "The camera is a critical part of the HelloRecipes app!", message: "Denying camera usage doesn't leave much functionality for HelloRecipes. Please go to settings and allow camera usage!", preferredStyle: .alert)
        alert.view.tintColor = uiColors.primary
